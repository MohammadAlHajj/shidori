...
public class ShidoriGUI extends JFrame 
{
	...
	public ShidoriGUI() 
	{
		...
		episodeList.addMouseListener(new MouseAdapter(){
			...
			public void mouseClicked(MouseEvent e)
			{
				...
				if(e.getClickCount()==2)
				{
					...
					try {
						desktop.open(listModel.get(episodeList.getSelectedIndex
								()).getFile());
					} 
		...
		// Button location
		{
			btnAddSeason.setBounds(1109, 66, 115, 23);
			btnAddEpisode.setBounds(1109, 100, 115, 23);
			btnAddShow.setBounds(1109, 32, 115, 23);
			btnRefresh.setBounds(1109, 168, 115, 23);
		}
		...
		//Button Action listeners
		{
			btnAddShow.addActionListener(new ActionListener() {
				...
				{
					AddComponent aC = shidori.addComponentMethod(TypeHolderEnum.
							Show);
				}
			});
...
//		        };
		...
		tree.addTreeSelectionListener(new TreeSelectionListener() 
		{
			...
			public void valueChanged(TreeSelectionEvent e) 
			{
				...
				else if(nodeInfo instanceof Season)
				{
					...
					for(MainObject episode : season.getEpisodes())
					{
						listModel.addElement((Episode)episode);
					}
				}
		...
		progressBar.setBounds(539, 394, 210, 14);
